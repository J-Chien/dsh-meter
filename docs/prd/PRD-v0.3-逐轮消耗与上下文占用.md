...
- **v0.3.5（交互与视觉规范统一）**：交互延迟集中到 `src/client/interaction.ts`（卡片与 tooltip 同源节奏）；原生 `title` 全部替换为统一 `Tooltip.tsx`；新增 `theme.module.css` 的 `--billing-*` token 层并让全部组件 CSS 走它；修复卡片与触发器间隙的悬停死区（P0）；恢复占用行的输出上限展示；补齐按钮 aria-label、保存失败内联错误、capability 分隔符、catalog provider 并发、计价公式单点化（`priceRequest`）、投影 zod 收紧与 invariant 测试。
- **v0.3.6（横轴可读性 + 占用条口径修正）**：详情图表横轴改纯数字标签（按轮次 `12`、按请求 `12.3`，原「轮次 N」中文标签在固定柱宽下截成「轮次轮次轮次」）；柱宽 18→24px、抽稀阈值重定（>20 隔 1、>40 隔 4、首尾必标）；`request/context.contextWindow` 经 harness 类型确认是**输入+输出合计**窗口——原「最近一次请求输入占上下文 ≥30%」提示为混合口径、无行动意义，删除（`SINGLE_TURN_WARN_RATIO` 常量与 `turn.lastInput` 文案一并移除）；占用条保留 ≥85% 预警并在文档注明口径。
- **v0.3.7（压缩预测条 + 请求级密集图）**：折叠 `compaction/summary` → `compactions`（count/lastTime/lastShadowedTokens），投影 `stateVersion` 7；占用条 80% 处画压缩触发线（compaction-basic 默认 0.8），显示压缩历史与预估（粗估口径已在文案/README 注明）；「按请求」视图改 12px 密集柱宽（宏观趋势），标签抽稀更激进。
- **v0.3.8（预估模型修正 + 触发线口径 + 轮次边界分组）**：预估改**相邻轮次输入差分**模型（原「轮总输入水平值」误当增速，42% 时算出 1 轮即触发）；触发线确认为 compaction-basic 私有 patch 配置、运行时不可读，以「默认 80%」如实标注并改 1px 半透明 hairline 样式；按请求图每轮 step=1 加前导间距 + 描边强调且 `N.1` 标签必标。
