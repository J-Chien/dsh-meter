...
- **上下文占用条（压缩预测）**：模型上下文窗口（来自日志 `request/context`）与最近一次请求总输入已知时，显示占用比例（进度条 + 百分比 + `已用 / 窗口` + `输出上限`，≥85% 预警「接近上限，建议开新会话」）；任一缺省则不显示（不估算）。⚠️ 口径：`request/context.contextWindow` 是 provider 声明的**输入+输出合计**窗口（harness 定义），故进度条 = 最近一次请求输入 ÷ 输入+输出总窗口。
  ...
  - **压缩预估**：**每轮新增 = 该轮最后一个请求的总输入 − 上一轮最后一个请求的总输入**（快照差分，不区分缓存命中/未命中——缓存失效时未缓存输入会假性暴涨到 555K，但总输入快照不变，差分口径因此免疫缓存状态）；**增速只用已完成的轮次**（进行中的轮次快照会随每个请求增长，纳入会在轮边界跳变、轮内漂移）；**全历史与最近 10 轮两个窗口各取 trimmed mean（去掉最大最小各一个）后取较小者**——早期一次性装载（system prompt/首批上下文）抬高的全历史均值不会虚增预估，近期变轻也不会过度乐观；headroom = 触发线 − 最近请求总输入（含缓存的快照水平）；显示「按最近轮次净增 +X/轮，余量 Y，约 N 轮后触发压缩」（余量可心算验证：N ≈ Y/X）——单轮偏轻/偏重不影响预估（粗估：harness 计量的是全表面启发式估算，插件只能看到真实 usage；分母是输入+输出窗口），<3 个正净增或无余量时不显示。
- **每轮新增迷你图**：最近若干**轮**每轮一根**新增占用 token** 竖条（**最老轮在左、时间递增**；柱高 = 该轮最后一个请求总输入 − 上一轮最后一个请求总输入，**不区分缓存命中**——缓存失效时历史重放为未缓存输入，但总输入快照不变，此口径免疫缓存状态；第一轮无前驱不计），高峰轮次暖色着色——该轮输入按高峰计价；hover 出统一 tooltip（轮次/新增占用/费用消耗/命中率）；柱宽固定 18px、`space-between` 两端贴边对称分布，**按卡片实际宽度自动算出能容纳的轮数**（ResizeObserver 实时重算，宽度变化/会话切换自适应），不再写死 10 轮
